...
import { X, QrCode, AlertCircle, FlipHorizontal, Loader2, ImagePlus, CheckCircle2, Bug, KeyRound } from 'lucide-react';
import { useQrScanner } from '../../hooks/useQrScanner';
import { formatAccessKey } from '../../services/nfceClassifier';
      ...
      <div className="bg-white dark:bg-slate-900 rounded-3xl max-w-sm sm:max-w-md w-full p-4 sm:p-5 shadow-2xl border border-slate-200 dark:border-slate-800 flex flex-col space-y-3 animate-in zoom-in-95 max-h-[92vh] overflow-y-auto">
        ...
        <div className="flex items-center justify-center gap-1.5 flex-wrap">
          <button
            ...
            className="px-2.5 py-1.5 bg-slate-100 dark:bg-slate-800 hover:bg-slate-200 dark:hover:bg-slate-700 text-slate-700 dark:text-slate-300 rounded-xl text-xs font-semibold flex items-center gap-1 transition-colors"
          >
            <FlipHorizontal className="w-3.5 h-3.5" />
            <span>Câmera</span>
          </button>

          <label className="px-2.5 py-1.5 bg-emerald-50 dark:bg-emerald-950/60 hover:bg-emerald-100 dark:hover:bg-emerald-900/60 text-emerald-700 dark:text-emerald-300 border border-emerald-200 dark:border-emerald-800 rounded-xl text-xs font-semibold flex items-center gap-1 cursor-pointer transition-colors">
          ...
          {onSwitchToKey && (
            <button
              type="button"
              onClick={onSwitchToKey}
              className="px-2.5 py-1.5 bg-indigo-50 dark:bg-indigo-950/60 hover:bg-indigo-100 dark:hover:bg-indigo-900/60 text-indigo-700 dark:text-indigo-300 border border-indigo-200 dark:border-indigo-800 rounded-xl text-xs font-semibold flex items-center gap-1 transition-colors"
            >
        ...
        <form onSubmit={handleManualSubmit} className="space-y-1.5 pt-1 border-t border-slate-100 dark:border-slate-800">
          <div className="flex items-center justify-between">
            ...
            {manualInput.replace(/\D/g, '').length > 0 && (
              <span className="text-[10px] font-mono text-slate-400">
                {manualInput.replace(/\D/g, '').length}/44
              </span>
            )}
          </div>
          <div className="flex space-x-2">
            <input
              ...
              onChange={(e) => {
                ...
                if (/^[\d\s]+$/.test(val)) {
                  setManualInput(formatAccessKey(val));
                } else {
                  setManualInput(val);
                }
              }}
              placeholder="5226 0906... ou https://nfeweb..."
              className="flex-1 px-3 py-2 bg-slate-50 dark:bg-slate-800 border border-slate-300 dark:border-slate-700 rounded-xl text-xs font-mono focus:outline-none focus:ring-2 focus:ring-emerald-500"
            />
            <button
              type="submit"
